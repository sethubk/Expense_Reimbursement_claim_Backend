...
namespace Claim_Form.Repositories.Implementations
{
    public class DomesticRepository:IDomesticRepository
    {
        ...
        {
            await _context.Domestic.AddRangeAsync(expenses);
        ...
        {
            return await _context.Domestic
                .AsNoTracking()
        ...
        {
            _context.Domestic.Update(expense);
            await _context.SaveChangesAsync();
        }
        ...
        {
            var expense = await _context.Domestic.FindAsync(id);

            if (expense == null)
                return;

            _context.Domestic.Remove(expense);
            await _context.SaveChangesAsync();
        }
    }
